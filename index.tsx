...
const App = () => {
  const [data, setData] = useState(() => {
    try {
      const saved = localStorage.getItem('lucano_v22_prod');
  ...
  useEffect(() => {
    localStorage.setItem('lucano_v22_prod', JSON.stringify(data));
  }, [data]);
  ...
  // SALVAR PROJETO: Zera cronômetro, para alarme e cria histórico
  const handleSave = () => {
    if (data.seconds < 1) return alert("Inicie o cronômetro para poder salvar.");
  ...
  // CONTINUAR TRABALHO: Retoma contagem do tempo salvo
  const resumeHistory = (h: any) => {
    setData((p: any) => ({
      ...
      rate: Number(h.rate),
      seconds: Number(h.time), // Continua de onde parou
      active: true // Inicia imediatamente
    }));
  ...
  // Exportação para Word (.doc formatado como texto rico)
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-[#f8fafc] text-slate-900 font-sans overflow-hidden select-none">
      ...
      {/* NAVEGAÇÃO */}
      <nav className="flex bg-white border-b border-slate-300 z-20 shadow-sm">
        ...
        {activeTab === 'controle' && (
          <div className="space-y-5 animate-in">
            ...
            <div className={`bg-[#334155] rounded-[2.5rem] p-10 text-center shadow-2xl border border-white/10 relative transition-all duration-500 ${isAlarmActive ? 'ring-[12px] ring-red-500 animate-pulse' : ''}`}>
               <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] block mb-2">Monitor de Tempo</span>
               ...
               <div className="grid grid-cols-2 gap-4">
                  <button onClick={handleSave} className="bg-[#e74c3c] text-white py-5 rounded-2xl font-black text-[11px] tracking-widest flex items-center justify-center gap-3 shadow-xl active:scale-95 border-2 border-white/20 transition-all">
                    <i className="fas fa-save"></i> SALVAR PROJETO
                  </button>
                  <button onClick={() => data.history[0] && resumeHistory(data.history[0])} className="bg-[#3498db] text-white py-5 rounded-2xl font-black text-[11px] tracking-widest flex items-center justify-center gap-3 shadow-xl active:scale-95 transition-all">
            ...
            <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-200 space-y-5">
               ...
               <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase ml-1 block">Notas do Projeto (Cores, MDF, Medidas)</label>
                  <textarea className="w-full bg-[#f8fafc] p-4 rounded-2xl border border-slate-200 text-xs text-slate-600 font-medium h-24 outline-none resize-none" placeholder="Detalhes aqui..." value={data.notes} onChange={e => setData({...data, notes: e.target.value})} />
               </div>
               <div className="bg-[#f8fafc] p-5 rounded-[2rem] border border-slate-200 text-center">
                  <span className="text-[9px] font-black text-slate-500 uppercase block mb-3 tracking-widest">Valor da Hora do Serviço</span>
          ...
          <div className="space-y-8 animate-in pb-20">
            ...
            {selectedIds.length > 0 && (
              <div className="bg-white rounded-xl border border-slate-400 shadow-2xl overflow-hidden mb-12 animate-in">
                 <div className="header-premium py-6">
                    <h2 className="text-xl font-black italic tracking-tighter">{BRAND_NAME}</h2>
                       ...
                       <tbody>
                          {selectedProjects.map(p => (
                             <tr key={p.id}>
                                ...
                                <td className="text-[10px]">{p.startTime.split(',')[0]}</td>
                                <td>{cur(p.rate)}</td>
                 ...
                 <div className="grid grid-cols-3 gap-2 p-3 bg-slate-100 border-t border-slate-300">
                    ...
                    <button onClick={() => exportWord(generateSumText(), 'Relatorio_Soma')} className="bg-[#3498db] text-white p-3 rounded-xl text-[10px] font-black uppercase shadow-md flex items-center justify-center gap-2"><i className="fas fa-file-word"></i> Word</button>
                    <button onClick={() => setSelectedIds([])} className="bg-slate-500 text-white p-3 rounded-xl text-[10px] font-black uppercase shadow-md">Limpar Seleção</button>
                 </div>
              </div>
            )}
            ...
            <div className="space-y-12">
              {data.history.length === 0 && (
                <div className="text-center py-20 opacity-30 font-black uppercase tracking-widest text-xs">Nenhum histórico disponível</div>
              )}
              {data.history.map((h: any) => (
                <div key={String(h.id)} className="relative">
                  {/* QUADRINHO DE SELEÇÃO DE PROJETO */}
                  <div className="absolute top-4 left-[-24px] z-10">
                    <input type="checkbox" className="cb-custom" checked={selectedIds.includes(h.id)} onChange={() => toggleSelect(h.id)} />
                  </div>

                  <div className="bg-white rounded-2xl border border-slate-400 shadow-xl overflow-hidden group hover:border-slate-500 transition-all">
                    <div className="header-premium py-6">
                      ...
                      <p className="text-[9px] text-slate-300">Tel / <span className="text-[#27ae60]">WhatsApp:</span> 74 9 91108629</p>
                    </div>
                    
                    <h3 className="text-center py-5 font-black text-[11px] text-slate-500 border-b border-slate-200 uppercase tracking-[0.2em] bg-slate-50">Relatório Individual de Produção</h3>
                    ...
                    <div className="p-6">
                      ...
                      {h.notes && (
                        <div className="mt-5 p-4 bg-slate-50 rounded-xl border border-slate-200">
                          <span className="text-[9px] font-black text-slate-400 block mb-2 uppercase tracking-widest">Notas Técnicas Adicionais:</span>
                          <p className="text-[11px] text-slate-600 italic leading-relaxed">{h.notes}</p>
                        </div>
                      )}
                      ...
                      <div className="grid grid-cols-4 gap-3 mt-8 border-t border-slate-100 pt-6">
                        <button onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(generateIndividualText(h))}`)} className="flex flex-col items-center justify-center p-3 rounded-2xl hover:bg-emerald-50 text-[#27ae60] transition-colors">
                          ...
                          <span className="text-[8px] font-black uppercase mt-2">WhatsApp</span>
                        </button>
                        ...
                        <button onClick={() => confirm("Apagar este registro?") && setData((p:any) => ({...p, history: p.history.filter((x:any) => x.id !== h.id)}))} className="flex flex-col items-center justify-center p-3 rounded-2xl hover:bg-red-50 text-red-500 transition-colors">
      ...
      <footer className="p-4 bg-white border-t border-slate-300 text-center z-10 shadow-inner">
         <div className="text-[8px] font-black text-slate-400 uppercase tracking-[0.3em] leading-relaxed">
           LUCANO DESIGNER3D V20.0 - GESTÃO PREMIUM PRO
         </div>
      </footer>
    </div>
  );
