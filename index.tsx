...
const App = () => {
  ...
  useEffect(() => {
    localStorage.setItem('lucano_v36_final', JSON.stringify(data));
  }, [data]);
  ...
  const handleSave = () => {
    if (data.seconds < 1) return alert("Inicie o cronômetro antes de finalizar.");

    const now = new Date();
    const entry = {
      id: Date.now().toString(),
      project: String(data.project || "PROJETO NOVO"),
  ...
  // CONTINUAR DO HISTÓRICO (CARREGA E DÁ PLAY)
  const resumeFromHistory = (h: any) => {
    setData((p: any) => ({
      ...
      active: true // Inicia contagem na hora
    }));
    setLastAlarmedAt(null);
    setShowAlarmChoice(false);
  ...
  // AÇÕES DO ALERTA (MODAL)
  const handleContinueAlarm = () => {
    setShowAlarmChoice(false);
    setData(prev => ({ ...prev, active: true })); // Volta a contar
    playSound('start');
  };

  const handleStopAlarm = () => {
    setShowAlarmChoice(false);
    setData(prev => ({ ...prev, active: false })); // Mantém parado
    playSound('stop');
  };
    ...
    let html = `
      ...
      <body style="font-family: Arial, sans-serif; background: #f1f5f9; padding: 40px;">
        ...
        <table border="1" cellspacing="0" cellpadding="12" style="width: 100%; border-collapse: collapse; margin-top: 25px; border: 1px solid #cbd5e1;">
          ${tableRows.map(row => `
            <tr>
              <td style="background: #f8fafc; font-weight: bold; width: 35%; color: #475569;">${row[0]}</td>
              <td style="background: #fff; color: #1e293b;">${row[1]}</td>
            </tr>
          `).join('')}
        </table>
        <div style="margin-top: 40px; font-weight: bold; text-align: right; border-top: 1px solid #cbd5e1; padding-top: 15px; font-size: 18px; color: #0891b2;">
          ${footerInfo}
        </div>
      </body></html>`;
    ...
    <div className="flex flex-col h-screen w-full bg-[#020617] text-slate-100 overflow-hidden select-none font-sans">
      ...
      <main className="flex-1 overflow-y-auto px-6 py-8 no-scrollbar relative bg-[radial-gradient(circle_at_50%_50%,_#0f172a_0%,_#020617_100%)]">
        ...
        {showAlarmChoice && (
          <div className="absolute inset-0 z-50 flex items-center justify-center p-8 bg-black/90 backdrop-blur-md">
             <div className="bg-slate-900 border-2 border-amber-500 rounded-[3rem] p-12 text-center shadow-[0_0_80px_rgba(245,158,11,0.4)] animate-in max-w-sm w-full">
                <div className="w-24 h-24 bg-amber-500/10 rounded-full flex items-center justify-center mx-auto mb-8 border border-amber-500/30">
                  <i className="fas fa-hourglass-end text-amber-500 text-4xl animate-bounce"></i>
                </div>
                <h2 className="text-2xl font-black text-white uppercase mb-4 leading-tight">Meta Concluída!</h2>
                <p className="text-slate-400 text-sm mb-10 font-medium italic">O ciclo de {data.goalMinutes} min encerrou.<br/>Deseja continuar a contagem?</p>
                <div className="flex flex-col gap-4">
                   <button onClick={handleContinueAlarm} className="w-full bg-emerald-500 hover:bg-emerald-400 text-white font-black py-5 rounded-2xl text-[11px] uppercase tracking-widest shadow-lg active:scale-95 transition-all">
                      CONTINUAR TRABALHO
                   </button>
                   <button onClick={handleStopAlarm} className="w-full bg-slate-800 hover:bg-rose-600 text-slate-300 hover:text-white font-black py-5 rounded-2xl text-[11px] uppercase tracking-widest active:scale-95 transition-all">
                      PARAR POR AGORA
                   </button>
                </div>
             </div>
        ...
        {activeTab === 'controle' && (
          <div className="space-y-8 animate-in max-w-2xl mx-auto">
            ...
            {/* CENTRAL DE TEMPO (CRONÔMETRO) */}
            <div className="bg-slate-900/60 backdrop-blur-2xl rounded-[3.5rem] p-12 text-center shadow-2xl border border-white/5 relative overflow-hidden group">
                <div className="absolute -top-10 -right-10 w-40 h-40 bg-cyan-500/5 rounded-full blur-3xl"></div>
                
                <span className="text-[10px] font-black text-cyan-400 uppercase tracking-[0.4em] mb-4 block opacity-60">Engine de Precisão v36</span>
                <div className="text-8xl font-black font-mono text-white tracking-tighter my-4 drop-shadow-[0_0_15px_rgba(255,255,255,0.2)]">
                  {formatT(data.seconds)}
                </div>
                <div className="text-4xl font-black text-emerald-400 italic mb-12 flex items-center justify-center gap-2">
                   <span className="text-xs text-slate-500 not-italic font-bold uppercase tracking-widest">Ganho:</span> {cur((data.seconds / 3600) * data.rate)}
                </div>

                <div className="grid grid-cols-2 gap-5 mb-5">
                  <button onClick={() => { setData(d => ({...d, active: true})); playSound('start'); setShowAlarmChoice(false); }} className="h-16 bg-emerald-500 hover:bg-emerald-400 rounded-2xl font-black text-white text-[11px] uppercase tracking-widest shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2">
                    <i className="fas fa-play"></i> INICIAR
                  </button>
                  <button onClick={() => { setData(d => ({...d, active: false})); playSound('stop'); }} className="h-16 bg-amber-500 hover:bg-amber-400 rounded-2xl font-black text-white text-[11px] uppercase tracking-widest shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2">
                    <i className="fas fa-pause"></i> PAUSAR
                  </button>
                </div>

                <div className="grid grid-cols-2 gap-5">
                  <button onClick={handleSave} className="h-16 bg-rose-600 hover:bg-rose-500 rounded-2xl font-black text-white text-[11px] uppercase tracking-widest shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2">
                    <i className="fas fa-check-double"></i> FINALIZAR & NOVO
                  </button>
                  <button onClick={() => data.history[0] && resumeFromHistory(data.history[0])} className="h-16 bg-slate-800 hover:bg-slate-700 rounded-2xl font-black text-white text-[11px] uppercase tracking-widest active:scale-95 transition-all">
                    RETOMAR ÚLTIMO
                  </button>
                </div>

                <div className="mt-10 pt-10 border-t border-white/5 flex items-center justify-center gap-6">
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Aviso de Meta (Minutos):</span>
                  <input type="number" className="w-20 bg-black/60 text-center font-black text-cyan-400 border border-white/10 rounded-xl p-3 outline-none focus:ring-1 ring-cyan-500/50 transition-all" value={data.goalMinutes} onChange={e => setData(d => ({...d, goalMinutes: parseInt(e.target.value) || 0}))} />
                </div>
            </div>
            ...
            <div className="bg-slate-900/40 backdrop-blur-md p-10 rounded-[3rem] border border-white/5 space-y-8 shadow-xl">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input className="w-full bg-black/40 p-5 rounded-2xl border border-white/10 text-white font-bold uppercase text-xs outline-none focus:border-cyan-500 transition-all" placeholder="Nome do Cliente..." value={data.client} onChange={e => setData(d => ({...d, client: e.target.value}))} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[9px] font-black text-slate-500 uppercase ml-2 tracking-widest">Projeto / Ambiente</label>
                    <input className="w-full bg-black/40 p-5 rounded-2xl border border-white/10 text-white font-bold uppercase text-xs outline-none focus:border-cyan-500 transition-all" placeholder="Ex: Cozinha Gourmet..." value={data.project} onChange={e => setData(d => ({...d, project: e.target.value}))} />
                  </div>
                </div>

                {/* CONTROLE DE VALOR DA HORA */}
                <div className="bg-slate-950/80 p-8 rounded-[2.5rem] border border-white/10 shadow-inner">
                   ...
                   <input type="range" min="1" max="1000" step="5" className="w-full h-2 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-emerald-500 shadow-md" value={data.rate} onChange={e => setData(d => ({...d, rate: parseInt(e.target.value)}))} />
                </div>

                <div className="space-y-2">
                  <label className="text-[9px] font-black text-slate-500 uppercase ml-2 tracking-widest">Anotações e Detalhes</label>
          ...
          <div className="space-y-10 pb-24 animate-in max-w-4xl mx-auto">
            ...
            {selectedIds.length > 0 && (
              <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-[3rem] border-l-[12px] border-emerald-500 p-10 shadow-2xl">
                 ...
                 <button onClick={() => {
                    const rows = selectedProjects.map(p => [`PROJETO: ${p.project} | CLIENTE: ${p.client}`, `TEMPO: ${formatT(p.time)} | TOTAL: ${cur(p.total)}`]);
                    exportWord("RELATÓRIO DE SOMA CONSOLIDADA", rows, `VALOR TOTAL ACUMULADO: ${cur(sumTotal)}`, "Soma_Unificada_Designer");
                    ...
                    <i className="fas fa-file-word"></i> Gerar Relatório de Soma Unificada
                 </button>
              </div>
            )}
                ...
                <div key={h.id} className="relative group pl-12">
                  ...
                  <div className={`bg-slate-900/80 rounded-[3rem] border transition-all duration-500 overflow-hidden ${selectedIds.includes(h.id) ? 'border-cyan-500 shadow-[0_0_40px_rgba(34,211,238,0.1)]' : 'border-white/5 hover:border-white/10 shadow-2xl'}`}>
                    <div className="bg-slate-800/60 p-8 flex justify-between items-center border-b border-white/5">
                       ...
                       <span className="bg-emerald-500/10 text-emerald-400 px-6 py-2.5 rounded-full text-[11px] font-black uppercase border border-emerald-500/20 shadow-inner">
                          {cur(h.total)}
                       </span>
                    </div>
                    ...
                    <div className="p-10 space-y-8">
                       <div className="grid grid-cols-2 gap-8">
                          <div>
                             <span className="text-[9px] font-black text-slate-500 uppercase block mb-2 tracking-widest">Cliente / Solicitante</span>
                             <span className="text-sm font-black text-white uppercase">{h.client}</span>
                          </div>
                          <div>
                             <span className="text-[9px] font-black text-slate-500 uppercase block mb-2 tracking-widest">Projeto / Ambiente</span>
                       ...
                       <div className="grid grid-cols-2 gap-8 pt-8 border-t border-white/5">
                          <div className="space-y-1">
                             <span className="text-[9px] font-black text-slate-500 uppercase block tracking-widest">Tempo Total Trabalhado</span>
                          ...
                          <div className="flex gap-3 items-center justify-end">
                             ...
                             <button onClick={() => {
                                exportWord("RELATÓRIO INDIVIDUAL", [
                                  ["CLIENTE", h.client],
                                  ["PROJETO", h.project],
                                  ...
                                  ["DATA", h.date],
                                  ["NOTAS", h.notes || "-"]
                                ], `VALOR TOTAL DO TRABALHO: ${cur(h.total)}`, `Relatorio_${h.project}`);
                             }} className="p-4 bg-slate-800 hover:bg-slate-700 text-cyan-400 rounded-2xl border border-white/5 transition-all active:scale-95 shadow-md">
                                <i className="fas fa-file-word text-lg"></i>
                             </button>
                             <button onClick={() => confirm("Deseja deletar permanentemente?") && setData(d => ({...d, history: d.history.filter((x:any) => x.id !== h.id)}))} className="p-4 bg-rose-600/10 hover:bg-rose-600 text-rose-500 hover:text-white border border-rose-500/30 rounded-2xl transition-all active:scale-95 shadow-md">
      ...
      <footer className="p-5 bg-slate-950 border-t border-white/5 text-center relative z-20 shadow-[0_-4px_30px_rgba(0,0,0,0.5)]">
         <div className="text-[8px] font-black text-slate-600 uppercase tracking-[0.5em] opacity-40">
           LUCANO DESIGNER3D PRO V36.0 • FLUXO OPERACIONAL FINAL
         </div>
      </footer>
    </div>
  );
